...
| TrailID | *string | Trail scope; nil for global stashes |
...
1.3. TrailID is optional. When nil, the stash is global (accessible from any trail). When set, the stash is scoped to that trail.
...
| --- | ---------- | ----------- |
| "trail_id" | *string | Match stashes in this trail (nil for global) |
...
- [ ] Stash struct defined with StashID, TrailID, Name, StashType, Value, Version, CreatedAt fields
...
- Stash→trail scope uses the links table with `scoped_to` link type (see ARCHITECTURE Decision 10). Global stashes have no `scoped_to` link. This keeps all relationships in one consistent pattern.
