...
1.2. PropertyID must be a UUID v7 (time-ordered) generated by the backend when Table.Set is called with an empty PropertyID.
...
2.2. CategoryID must be a UUID v7 (time-ordered) generated by the backend when Table.Set is called with an empty CategoryID.
...
3.3. CrumbTable.SetProperty validates that values match the property's ValueType (see prd-crumbs-interface R5).
...
4.4. After successful creation, the Property struct is updated with the generated PropertyID and CreatedAt timestamp.
...
7.6. DefineCategory must persist the category via cupboard.GetTable("categories").Set("", category).
...
7.9. DefineCategory requires the Cupboard reference to access the categories table.
...
8.3. GetCategories must query the categories table with a filter for PropertyID.
...
8.6. GetCategories requires the Cupboard reference to access the categories table.
